# ~~stolen fr-~~ inspired by github:shivajreddy/dotfiles
{
  config,
  lib,
  pkgs,
  ...
  # NOTE 3: make sure to give an empty table as the input for 'ensure_installed' in the nvim-treesitter setup, that
  ...
  # NOTE 4: This makes our custom lua config files, which is the beauty of just using lua files the regular way.
  ...
  # unlike the way Shiva does it, just leave the nvim config where it was.
  # This makes LazyVim autoreload config, fixes <leader>fc and possibly more
